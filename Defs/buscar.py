...
def buscar_aluno():
    ...
    resultados = cur.execute(
        f"SELECT * FROM tb_aluno WHERE lower(nome) LIKE ? OR id = ?",
        (f"%{busca.lower()}%", busca),
    )
...
def buscar_usuario():
    ...
    resultados = cur.execute(
        f"SELECT * FROM tb_usuario WHERE lower(nome) LIKE ? OR id = ?",
        (f"%{busca.lower()}%", busca),
    )
...
def buscar_funcionario():
    ...
    busca = input("Digite um nome ou id para pesquisar=> ")
    resultados = cur.execute(
        f"SELECT * FROM tb_funcionario WHERE lower(nome) LIKE ? OR id = ?",
        (f"%{busca.lower()}%", busca),
    )
...
def buscar_cargo():
    ...
    resultados = cur.execute(
        f"SELECT *FROM tb_cargo WHERE codigo = ? OR lower(nome) LIKE ? OR setor = ?",
        (busca, f"%{busca.lower()}%", busca),
    )
...
def buscar_plano():
    ...
    resultados = cur.execute(
        f"SELECT *FROM tb_plano WHERE codigo = ? or lower(descricao) LIKE ?  or lower(nivel) LIKE ?",
        (busca, f"%{busca.lower()}%", f"%{busca.lower()}%"),
    )
