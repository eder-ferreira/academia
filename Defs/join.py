...
def join_usuario_aluno_endereco():
    ...
    cur.execute(
        """SELECT tb_usuario.matricula, tb_usuario.nome, 
...
def salarios():
    ...
    cur.execute(
        f""" SELECT nome, setor, salario_base FROM tb_cargo ORDER BY salario_base DESC LIMIT {escolha} """
    )
...
def aniversario():
    ...
    cur.execute(
        f"""SELECT nome, dt_nascimento, strftime('%Y', 'now') - strftime('%Y', dt_nascimento) AS idade FROM 
...
def gera_grafico_salario():
    cur.execute(
        f""" SELECT setor, salario_base FROM tb_cargo ORDER BY salario_base DESC LIMIT {escolha} """
    )
    resultados = cur.fetchall()
